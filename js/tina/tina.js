...
var Tina = function(ops) {
  ...
  T.setup = function(id,options) {
    ...
    T.ctx = T.cvs.getContext &&
      T.cvs.getContext("2d");
    T.cvs.width = Math.min(options.width || 420, 4096);
    T.cvs.height = Math.min(options.height || 320, 4096);
    ...
    options.scale = options.scale || {x:_pixelRatio,y:_pixelRatio};
    T.scale = options.scale;

    T.width  = parseInt(T.cvs.width/T.scale.x,10);
  ...
  T.audioMimeTypes = { mp3: "audio/mpeg", 
  ...
  T.load = function(assets,callback,ops) {
    ...
    var progressCallback = ops.progressCallback || function(status,total) {
      ...
      var w = T.width * 0.6;
      var h = T.height * 0.04;
      var x = T.width * 0.5 - w/2;
      var y = T.height * 0.618 - h/2;

      T.clear( T.options.clearColor );
	    T.ctx.save();
      if(T.scale)
        T.ctx.scale(T.scale.x,T.scale.y);
      
		  T.ctx.fillStyle = '#fff';
		  T.ctx.fillRect( x*s-1, y*s-1, w*s+2, h*s+2 );
		  
		  T.ctx.fillStyle = '#000';
		  T.ctx.fillRect( x*s+s, y*s+s, w*s-s-s, h*s-s-s );
		  
		  T.ctx.fillStyle = '#fff';
		  T.ctx.fillRect( x*s+4, y*s+4, w*s*status/total-8, h*s-8 );
