Tina.Scenes = function(T) {
  ...
  T.scene = function(name,sceneObj) {
    if(sceneObj) { T.scenes[name] = sceneObj; }
  ...
  T.Stage = T.GameObject.extend({
    ...
    init: function(scene) {
      ...
      if(this.options.sort && !_.isFunction(this.options.sort)) {
        this.options.sort = function(a,b) { return a.z - b.z; };
      }
    },
  ...
  T.c("interactive",{
    ...
    getInputPoint: function(e,element) {
      var point = {x: (e.pageX || e.clientX + document.body.scrollLeft) - element.offsetLeft,
